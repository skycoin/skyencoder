...
func FindStructInfoInProgram(p *loader.Program, name string) (*StructInfo, error) {
	...
	for _, pk := range p.Created {
		s, err := findStructInPackage(pk, name)
		if err != nil {
			return nil, err
		}
		if s != nil {
...
func BuildStructEncoderTest(s *StructInfo, destPackage, fmtFilename string) ([]byte, error) {
	pkgName := ""
	if destPackage != "" {
		pkgName = s.Package.Name()
	} else {
	...
	src := buildTest(s.Name, pkgName, destPackage, hm)
