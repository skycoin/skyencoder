...
func testSkyencoderMaxLenStringStruct2(t *testing.T, obj *MaxLenStringStruct2) {
	...
	// Encode
	data2, err := EncodeMaxLenStringStruct2(obj)
	if err != nil {
		t.Fatalf("EncodeMaxLenStringStruct2 failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("EncodeMaxLenStringStruct2 produced bytes of unexpected length")
	}
	...
	if err := EncodeMaxLenStringStruct2ToBuffer(data3, obj); err != nil {
		t.Fatalf("EncodeMaxLenStringStruct2ToBuffer failed: %v", err)
	}
	...
	var obj4 MaxLenStringStruct2
	n, err := DecodeMaxLenStringStruct2(data3, &obj4)
	if err != nil {
		t.Fatalf("DecodeMaxLenStringStruct2 failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("DecodeMaxLenStringStruct2 bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("DecodeMaxLenStringStruct2 bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 MaxLenStringStruct2
	if err := DecodeMaxLenStringStruct2Exact(data2, &obj5); err != nil {
		t.Fatalf("DecodeMaxLenStringStruct2 failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := DecodeMaxLenStringStruct2(data4, &obj3); err != nil {
			t.Fatalf("DecodeMaxLenStringStruct2 failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("DecodeMaxLenStringStruct2 bytes read length should be %d, is %d", len(data2), n)
		}
...
func decodeMaxLenStringStruct2ExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj MaxLenStringStruct2
	if err := DecodeMaxLenStringStruct2Exact(buf, &obj); err == nil {
		t.Fatal("DecodeMaxLenStringStruct2Exact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("DecodeMaxLenStringStruct2Exact: expected error %q, got %q", expectedErr, err)
	}
}
...
func testSkyencoderMaxLenStringStruct2DecodeErrors(t *testing.T, k int, tag string, obj *MaxLenStringStruct2) {
	...
	buf, err := EncodeMaxLenStringStruct2(obj)
	if err != nil {
		t.Fatalf("EncodeMaxLenStringStruct2 failed: %v", err)
	}
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeMaxLenStringStruct2ExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeMaxLenStringStruct2ExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeMaxLenStringStruct2ExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}
