...
func testSkyencoderMaxLenNestedMapValueStruct2(t *testing.T, obj *MaxLenNestedMapValueStruct2) {
	...
	// Encode
	data2, err := EncodeMaxLenNestedMapValueStruct2(obj)
	if err != nil {
		t.Fatalf("EncodeMaxLenNestedMapValueStruct2 failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("EncodeMaxLenNestedMapValueStruct2 produced bytes of unexpected length")
	}
	...
	if err := EncodeMaxLenNestedMapValueStruct2ToBuffer(data3, obj); err != nil {
		t.Fatalf("EncodeMaxLenNestedMapValueStruct2ToBuffer failed: %v", err)
	}
	...
	if n, err := DecodeMaxLenNestedMapValueStruct2(data2, &obj3); err != nil {
		t.Fatalf("DecodeMaxLenNestedMapValueStruct2 failed: %v", err)
	} else if n != uint64(len(data2)) {
		t.Fatalf("DecodeMaxLenNestedMapValueStruct2 bytes read length should be %d, is %d", len(data2), n)
	}
	...
	var obj4 MaxLenNestedMapValueStruct2
	n, err := DecodeMaxLenNestedMapValueStruct2(data3, &obj4)
	if err != nil {
		t.Fatalf("DecodeMaxLenNestedMapValueStruct2 failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("DecodeMaxLenNestedMapValueStruct2 bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("DecodeMaxLenNestedMapValueStruct2 bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 MaxLenNestedMapValueStruct2
	if err := DecodeMaxLenNestedMapValueStruct2Exact(data2, &obj5); err != nil {
		t.Fatalf("DecodeMaxLenNestedMapValueStruct2 failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := DecodeMaxLenNestedMapValueStruct2(data4, &obj3); err != nil {
			t.Fatalf("DecodeMaxLenNestedMapValueStruct2 failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("DecodeMaxLenNestedMapValueStruct2 bytes read length should be %d, is %d", len(data2), n)
		}
...
func decodeMaxLenNestedMapValueStruct2ExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj MaxLenNestedMapValueStruct2
	if _, err := DecodeMaxLenNestedMapValueStruct2(buf, &obj); err == nil {
...
func decodeMaxLenNestedMapValueStruct2ExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj MaxLenNestedMapValueStruct2
	if err := DecodeMaxLenNestedMapValueStruct2Exact(buf, &obj); err == nil {
		t.Fatal("DecodeMaxLenNestedMapValueStruct2Exact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("DecodeMaxLenNestedMapValueStruct2Exact: expected error %q, got %q", expectedErr, err)
	}
}

func testSkyencoderMaxLenNestedMapValueStruct2DecodeErrors(t *testing.T, k int, tag string, obj *MaxLenNestedMapValueStruct2) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
	...
	buf, err := EncodeMaxLenNestedMapValueStruct2(obj)
	if err != nil {
		t.Fatalf("EncodeMaxLenNestedMapValueStruct2 failed: %v", err)
	}
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeMaxLenNestedMapValueStruct2ExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeMaxLenNestedMapValueStruct2ExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeMaxLenNestedMapValueStruct2ExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}
...
func TestSkyencoderMaxLenNestedMapValueStruct2DecodeErrors(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		testSkyencoderMaxLenNestedMapValueStruct2DecodeErrors(t, i, "empty", emptyObj)
		testSkyencoderMaxLenNestedMapValueStruct2DecodeErrors(t, i, "full", fullObj)
	}
}
