...
func testSkyencoderOmitEmptyStruct(t *testing.T, obj *OmitEmptyStruct) {
	...
	// Encode
	data2, err := EncodeOmitEmptyStruct(obj)
	if err != nil {
		t.Fatalf("EncodeOmitEmptyStruct failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("EncodeOmitEmptyStruct produced bytes of unexpected length")
	}
	...
	if err := EncodeOmitEmptyStructToBuffer(data3, obj); err != nil {
		t.Fatalf("EncodeOmitEmptyStructToBuffer failed: %v", err)
	}
	...
	var obj4 OmitEmptyStruct
	n, err := DecodeOmitEmptyStruct(data3, &obj4)
	if err != nil {
		t.Fatalf("DecodeOmitEmptyStruct failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("DecodeOmitEmptyStruct bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("DecodeOmitEmptyStruct bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 OmitEmptyStruct
	if err := DecodeOmitEmptyStructExact(data2, &obj5); err != nil {
		t.Fatalf("DecodeOmitEmptyStruct failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := DecodeOmitEmptyStruct(data4, &obj3); err != nil {
			t.Fatalf("DecodeOmitEmptyStruct failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("DecodeOmitEmptyStruct bytes read length should be %d, is %d", len(data2), n)
		}
...
func decodeOmitEmptyStructExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj OmitEmptyStruct
	if err := DecodeOmitEmptyStructExact(buf, &obj); err == nil {
		t.Fatal("DecodeOmitEmptyStructExact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("DecodeOmitEmptyStructExact: expected error %q, got %q", expectedErr, err)
	}
}
...
func testSkyencoderOmitEmptyStructDecodeErrors(t *testing.T, k int, tag string, obj *OmitEmptyStruct) {
	...
	buf, err := EncodeOmitEmptyStruct(obj)
	if err != nil {
		t.Fatalf("EncodeOmitEmptyStruct failed: %v", err)
	}
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeOmitEmptyStructExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeOmitEmptyStructExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeOmitEmptyStructExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}
